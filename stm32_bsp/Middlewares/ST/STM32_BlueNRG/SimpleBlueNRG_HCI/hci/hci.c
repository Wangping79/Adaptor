...
//    Disable_SPI_IRQ();
  ...
  //TBD: How to transfer the data to VFD
//  Enable_SPI_IRQ();    
}
...
//  while(BlueNRG_DataPresent())
  {
    if (list_is_empty (&hciReadPktPool) == FALSE)
    {
      ...
      data_len = BlueNRG_SPI_Read_All(&hspi1_BlueBRG, hciReadPacket->dataBuff, HCI_READ_PACKET_SIZE);
      if(data_len > 0)
      {
...
//    Disable_SPI_IRQ();
    ...
    /* If there are no more packets to be processed, be sure there is at list one
       ...
       If no free packets are available, discard the processed event and insert it into the pool. */
...
int hci_le_set_advertising_data(uint8_t length, const uint8_t data[])
{
  ...
  Osal_MemCpy(adv_cp.data, data, HCI_MIN(31,length));
...
int hci_le_set_scan_resp_data(uint8_t length, const uint8_t data[])
{
  ...
  Osal_MemCpy(scan_resp_cp.data, data, HCI_MIN(31,length));
