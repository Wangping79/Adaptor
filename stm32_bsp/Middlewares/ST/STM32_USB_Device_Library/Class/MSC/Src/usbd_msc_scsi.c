...
static int8_t SCSI_TestUnitReady(USBD_HandleTypeDef  *pdev, uint8_t lun, uint8_t *params)
{
  ...
  if(((USBD_StorageTypeDef *)pdev->pUserData[MSC_USER_DATA])->IsReady(lun) !=0 )
  {
    SCSI_SenseCode(pdev,
                   lun,
                   NOT_READY, 
...
static int8_t  SCSI_Inquiry(USBD_HandleTypeDef  *pdev, uint8_t lun, uint8_t *params)
{
  ...
  else
  {
    ...
    pPage = (uint8_t *)&((USBD_StorageTypeDef *)pdev->pUserData[MSC_USER_DATA])->pInquiry[lun * STANDARD_INQUIRY_DATA_LEN];
    len = pPage[4] + 5;
...
static int8_t SCSI_ReadCapacity10(USBD_HandleTypeDef  *pdev, uint8_t lun, uint8_t *params)
{
  ...
  if(((USBD_StorageTypeDef *)pdev->pUserData[MSC_USER_DATA])->GetCapacity(lun, &hmsc->scsi_blk_nbr, &hmsc->scsi_blk_size) != 0)
  {
    SCSI_SenseCode(pdev,
                   lun,
                   NOT_READY, 
...
static int8_t SCSI_ReadFormatCapacity(USBD_HandleTypeDef  *pdev, uint8_t lun, uint8_t *params)
{
  ...
  if(((USBD_StorageTypeDef *)pdev->pUserData[MSC_USER_DATA])->GetCapacity(lun, &blk_nbr, &blk_size) != 0)
  {
    SCSI_SenseCode(pdev,
                   lun,
                   NOT_READY, 
...
static int8_t SCSI_Write10 (USBD_HandleTypeDef  *pdev, uint8_t lun , uint8_t *params)
{
  ...
  if (hmsc->bot_state == USBD_BOT_IDLE) /* Idle */
  {
    ...
    if(((USBD_StorageTypeDef *)pdev->pUserData[MSC_USER_DATA])->IsWriteProtected(lun) !=0 )
    {
      SCSI_SenseCode(pdev,
                     lun,
                     NOT_READY, 
...
static int8_t SCSI_ProcessRead (USBD_HandleTypeDef  *pdev, uint8_t lun)
{
  ...
  if( ((USBD_StorageTypeDef *)pdev->pUserData[MSC_USER_DATA])->Read(lun ,
                              hmsc->bot_data, 
                              hmsc->scsi_blk_addr / hmsc->scsi_blk_size, 
...
static int8_t SCSI_ProcessWrite (USBD_HandleTypeDef  *pdev, uint8_t lun)
{
  ...
  if(((USBD_StorageTypeDef *)pdev->pUserData[MSC_USER_DATA])->Write(lun ,
                              hmsc->bot_data, 
                              hmsc->scsi_blk_addr / hmsc->scsi_blk_size, 
                              len / hmsc->scsi_blk_size) < 0)
  {
